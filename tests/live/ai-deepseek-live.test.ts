...
import {createDeepSeekChatProviderFromProcessEnv} from "@/lib/ai/providers/deepseek-chat";
...
const live=process.env.RUN_LIVE_DEEPSEEK==="1";
const suite=live?describe:describe.skip;

suite("A3 live DeepSeek Chat acceptance",()=>{
  it("completes a real read-only turn and structured context tool loop without Project mutation or key persistence",async()=>{
    const apiKey=process.env.DEEPSEEK_API_KEY;
    const model=process.env.DEEPSEEK_MODEL;
    expect(apiKey).toBeTruthy();
    expect(model).toMatch(/^deepseek-v4-(?:flash|pro)$/);

    const now=new Date().toISOString();
    const project=createProject({id:"a3-live-deepseek",name:"A3 Live DeepSeek",now,durationInFrames:300});
    project.project.revision=11;
    const projectTruth=ProjectSchema.parse(project);
    const before=JSON.stringify(projectTruth);
    const fs=new InMemoryFileSystemAdapter();
    const sessions=new AgentSessionRepository(fs,"/a3-live-deepseek");
    const context=new AgentContextService({load:async()=>ProjectSchema.parse(projectTruth)});
    const provider=createDeepSeekChatProviderFromProcessEnv({timeoutMs:60_000});

    const readService=new AgentSessionService({
      provider,
      context,
    ...
    const toolService=new AgentSessionService({
      provider,
      context,
      tools:new AgentToolRegistry([createProjectContextReadTool()]),
      sessions,
    });
    ...
    const toolResult=await toolService.runTurn({
      ...
      userContent:"Mandatory acceptance protocol: call the only available function get_project_context exactly once before answering. Do not answer from existing context. After the tool result, reply with the Project revision.",
