...
describe("V2.2 W5 failure retry and restart hardening",()=>{
  ...
  it("serializes WorkflowService metadata writes with concurrent Runner state writes",async()=>{
    const root=await makeRoot();const storeA=new FileWorkflowStore(root);const storeB=new FileWorkflowStore(root);const flow=definition([stage("analysis")]);const definitionsA=new WorkflowDefinitionRegistry();const definitionsB=new WorkflowDefinitionRegistry();definitionsA.register(flow);definitionsB.register(flow);let calls=0;const register=(registry:WorkflowStageRegistry)=>registry.register("executor-analysis",{start:async()=>{calls++;await new Promise(resolve=>setTimeout(resolve,50));return{kind:"completed"};}});const stagesA=new WorkflowStageRegistry();const stagesB=new WorkflowStageRegistry();register(stagesA);register(stagesB);const project=createProject({id:"demo",name:"Demo",now:at,width:1920,height:1080,fps:30,durationInFrames:300});const runnerA=new WorkflowRunner(storeA,definitionsA,stagesA);const serviceA=new WorkflowService({load:async()=>project},storeA,definitionsA,runnerA);const created=await serviceA.create({projectId:"demo",definitionId:flow.id,definitionVersion:flow.version,sourceAssetIds:[],expectedProjectRevision:0});await storeA.save(WorkflowRunSchema.parse({...created,status:"running",stageExecutions:[{stageId:"analysis",status:"ready",attempt:0,jobIds:[],operationIds:[],artifactIds:[]}]}));const runnerB=new WorkflowRunner(storeB,definitionsB,stagesB);const serviceB=new WorkflowService({load:async()=>project},storeB,definitionsB,runnerB);(runnerA as unknown as {schedule:(workflowId:string)=>void}).schedule(created.id);await runnerB.recover();await Promise.all([serviceB.bindAssetBaseUrl(created.id,"http://127.0.0.1:3032"),runnerA.waitForIdle(created.id),runnerB.waitForIdle(created.id)]);const done=await storeA.get(created.id);expect(calls).toBe(1);expect(done?.status).toBe("completed");expect(done?.assetBaseUrl).toBe("http://127.0.0.1:3032");
  });
