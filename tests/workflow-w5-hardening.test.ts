...
describe("V2.2 W5 failure retry and restart hardening",()=>{
  ...
  it("serializes concurrent recovery writes from separate WorkflowRunner instances",async()=>{
    const root=await makeRoot();const storeA=new FileWorkflowStore(root);const storeB=new FileWorkflowStore(root);const flow=definition([stage("analysis")]);const definitionsA=new WorkflowDefinitionRegistry();const definitionsB=new WorkflowDefinitionRegistry();definitionsA.register(flow);definitionsB.register(flow);const stagesA=new WorkflowStageRegistry();const stagesB=new WorkflowStageRegistry();const project=createProject({id:"demo",name:"Demo",now:at,width:1920,height:1080,fps:30,durationInFrames:300});const runnerA=new WorkflowRunner(storeA,definitionsA,stagesA);const service=new WorkflowService({load:async()=>project},storeA,definitionsA,runnerA);const created=await service.create({projectId:"demo",definitionId:flow.id,definitionVersion:flow.version,sourceAssetIds:[],expectedProjectRevision:0});
    await storeA.save(WorkflowRunSchema.parse({...created,status:"running",currentStageId:"analysis",stageExecutions:[{stageId:"analysis",status:"running",attempt:1,attemptId:"22222222-2222-4222-8222-222222222222",startedAt:at,baseProjectRevision:0,jobIds:[],operationIds:["workflow:old-attempt"],artifactIds:[]}]}));
    const runnerB=new WorkflowRunner(storeB,definitionsB,stagesB);await Promise.all([runnerA.recover(),runnerB.recover()]);const recovered=await storeA.get(created.id);
    expect(recovered?.status).toBe("interrupted");expect(recovered?.stageExecutions[0]).toMatchObject({status:"interrupted",error:{code:"WORKFLOW_STAGE_INTERRUPTED",retryable:true}});
  });

  it("serializes WorkflowService metadata writes with concurrent Runner state writes",async()=>{
    const root=await makeRoot();const storeA=new FileWorkflowStore(root);const storeB=new FileWorkflowStore(root);const flow=definition([stage("analysis")]);const definitionsA=new WorkflowDefinitionRegistry();definitionsA.register(flow);let calls=0;const stagesA=new WorkflowStageRegistry();stagesA.register("executor-analysis",{start:async()=>{calls++;await new Promise(resolve=>setTimeout(resolve,50));return{kind:"completed"};}});const project=createProject({id:"demo",name:"Demo",now:at,width:1920,height:1080,fps:30,durationInFrames:300});const runnerA=new WorkflowRunner(storeA,definitionsA,stagesA);const serviceA=new WorkflowService({load:async()=>project},storeA,definitionsA,runnerA);const created=await serviceA.create({projectId:"demo",definitionId:flow.id,definitionVersion:flow.version,sourceAssetIds:[],expectedProjectRevision:0});const serviceB=new WorkflowService({load:async()=>project},storeB,definitionsA,{recover:async()=>{}} as unknown as WorkflowRunner);await storeA.save(WorkflowRunSchema.parse({...created,status:"running",stageExecutions:[{stageId:"analysis",status:"ready",attempt:0,jobIds:[],operationIds:[],artifactIds:[]}]}));(runnerA as unknown as {schedule:(workflowId:string)=>void}).schedule(created.id);await Promise.all([serviceB.bindAssetBaseUrl(created.id,"http://127.0.0.1:3032"),runnerA.waitForIdle(created.id)]);const done=await storeA.get(created.id);expect(calls).toBe(1);expect(done?.status).toBe("completed");expect(done?.assetBaseUrl).toBe("http://127.0.0.1:3032");
  });

  it("serializes cross-runner Workflow reads with state writes",async()=>{
    const root=await makeRoot();const storeA=new FileWorkflowStore(root);const storeB=new FileWorkflowStore(root);const flow=definition([stage("analysis")]);const definitions=new WorkflowDefinitionRegistry();definitions.register(flow);const stages=new WorkflowStageRegistry();let calls=0;stages.register("executor-analysis",{start:async()=>{calls++;await new Promise(resolve=>setTimeout(resolve,50));return{kind:"completed"};}});const project=createProject({id:"demo",name:"Demo",now:at,width:1920,height:1080,fps:30,durationInFrames:300});const runner=new WorkflowRunner(storeA,definitions,stages);const service=new WorkflowService({load:async()=>project},storeA,definitions,runner);const created=await service.create({projectId:"demo",definitionId:flow.id,definitionVersion:flow.version,sourceAssetIds:[],expectedProjectRevision:0});await storeA.save(WorkflowRunSchema.parse({...created,status:"running",stageExecutions:[{stageId:"analysis",status:"ready",attempt:0,jobIds:[],operationIds:[],artifactIds:[]}]}));(runner as unknown as {schedule:(workflowId:string)=>void}).schedule(created.id);const reads=Promise.all(Array.from({length:32},()=>storeB.get(created.id)));await Promise.all([reads,runner.waitForIdle(created.id)]);const done=await storeA.get(created.id);expect(calls).toBe(1);expect(done?.status).toBe("completed");expect(done?.stageExecutions[0].attempt).toBe(1);
  });
  ...
  it("claims a ready Stage once across WorkflowRunner instances sharing one data root",async()=>{
    const root=await makeRoot();const storeA=new FileWorkflowStore(root);const storeB=new FileWorkflowStore(root);const flow=definition([stage("analysis")]);const definitionsA=new WorkflowDefinitionRegistry();const definitionsB=new WorkflowDefinitionRegistry();definitionsA.register(flow);definitionsB.register(flow);let calls=0;
    const register=(registry:WorkflowStageRegistry)=>registry.register("executor-analysis",{start:async()=>{calls++;await new Promise(resolve=>setTimeout(resolve,40));return{kind:"completed"};}});
    const stagesA=new WorkflowStageRegistry();const stagesB=new WorkflowStageRegistry();register(stagesA);register(stagesB);const project=createProject({id:"demo",name:"Demo",now:at,width:1920,height:1080,fps:30,durationInFrames:300});const runnerA=new WorkflowRunner(storeA,definitionsA,stagesA);const service=new WorkflowService({load:async()=>project},storeA,definitionsA,runnerA);const created=await service.create({projectId:"demo",definitionId:flow.id,definitionVersion:flow.version,sourceAssetIds:[],expectedProjectRevision:0});await storeA.save(WorkflowRunSchema.parse({...created,status:"running",stageExecutions:[{stageId:"analysis",status:"ready",attempt:0,jobIds:[],operationIds:[],artifactIds:[]}]}));const runnerB=new WorkflowRunner(storeB,definitionsB,stagesB);(runnerA as unknown as {schedule:(workflowId:string)=>void}).schedule(created.id);(runnerB as unknown as {schedule:(workflowId:string)=>void}).schedule(created.id);await Promise.all([runnerA.waitForIdle(created.id),runnerB.waitForIdle(created.id)]);const done=await storeA.get(created.id);expect(calls).toBe(1);expect(done?.stageExecutions[0]).toMatchObject({status:"completed",attempt:1});
  });

  it("preserves human replay operation and Job history while clearing active attempt references",async()=>{
    const root=await makeRoot();const store=new FileWorkflowStore(root);const definitions=new WorkflowDefinitionRegistry();const flow=WorkflowDefinitionSchema.parse({id:"w5-replay-history",version:"1",name:"W5 Replay History",scenario:"talking-head",stages:[{id:"CONTENT_REVIEW",kind:"checkpoint",dependsOn:[],retryable:true,reviewRequired:true,invalidates:[],executorKey:"executor-review"},{id:"CAPTION_GENERATION",kind:"mutation",dependsOn:[],retryable:true,reviewRequired:false,invalidates:["CONTENT_REVIEW"],executorKey:"executor-caption"}],entryStageIds:["CONTENT_REVIEW","CAPTION_GENERATION"]});definitions.register(flow);const stages=new WorkflowStageRegistry();const runner=new WorkflowRunner(store,definitions,stages);const created=WorkflowRunSchema.parse({id:"88888888-8888-4888-8888-888888888888",definitionId:flow.id,definitionVersion:flow.version,projectId:"demo",createdAt:at,updatedAt:at,status:"waiting_review",scenario:"talking-head",sourceAssetIds:[],canvasSnapshot:{width:1920,height:1080,fps:30},stageExecutions:[{stageId:"CONTENT_REVIEW",status:"waiting_review",attempt:1,attemptId:"99999999-9999-4999-8999-999999999999",startedAt:at,baseProjectRevision:4,jobIds:[],operationIds:["review-op"],artifactIds:[]},{stageId:"CAPTION_GENERATION",status:"completed",attempt:1,attemptId:"aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa",startedAt:at,completedAt:at,baseProjectRevision:4,inputDigest:"caption-input",outputDigest:"caption-output",jobIds:["bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb"],operationIds:["caption-op"],artifactIds:[]}],checkpoints:[{id:"cccccccc-cccc-4ccc-8ccc-cccccccccccc",stageId:"CONTENT_REVIEW",status:"waiting_review",createdAt:at,baseProjectRevision:4}],artifacts:[],lastKnownProjectRevision:4});await store.create(created);const replayed=await runner.replayFromStage(created.id,"CAPTION_GENERATION",4);await runner.waitForIdle(created.id);const stored=await store.get(created.id);const execution=stored!.stageExecutions.find(item=>item.stageId==="CAPTION_GENERATION")!;expect(replayed.stageExecutions.find(item=>item.stageId==="CAPTION_GENERATION")?.status).toBe("invalidated");expect(["pending","ready"]).toContain(execution.status);expect(execution.jobIds).toEqual([]);expect(execution.operationIds).toEqual([]);expect(execution.historicalJobIds).toEqual(["bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb"]);expect(execution.historicalOperationIds).toEqual(["caption-op"]);
  });
});
