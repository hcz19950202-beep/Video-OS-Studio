...
const readContract=(toolId:string):SharedAgentToolContract=>SharedAgentToolContractSchema.parse({
  toolId,
  version:"1.0.0",
...
const registry=new SharedToolRegistry(TOOL_IDS.map((toolId):RegisteredSharedTool=>({
...
const createHarness=async()=>{
  ...
  const credential=controller.issueCredential({clientType:"test",clientLabel:"C4 test client"});
...
afterEach(async()=>{
  await Promise.all(running.splice(0).map(async harness=>{
...
const post=async(
  ...
  headers:{
    ...
    ...(options.token===undefined?{Authorization:`Bearer ${harness.credential.token}`}:
...
describe("V2.5 C4 Local MCP modern HTTP boundary",()=>{
  it("refuses non-loopback bind configuration before opening a server",async()=>{
    ...
    const server=new LocalMcpHttpServer(new LocalMcpBridgeController(contextService),registry);
  ...
  it("denies missing and invalid bearer credentials",async()=>{
    ...
    const invalid=await post(harness,"server/discover",{}, {token:"not-a-valid-local-mcp-secret-xxxxxxxxxxxxxxxx"});
  ...
  it("discovers a read-only bridge and lists exactly the C4 minimum R0 catalog",async()=>{
    ...
    const discovered=await discover.json() as {result:{capabilities:{tools:unknown};supportedVersions:string[]}};
    ...
    const payload=await listed.json() as {result:{tools:Array<{name:string;annotations:{readOnlyHint:boolean}}>}};
  ...
  it("calls an R0 tool only against the controller-bound active Project",async()=>{
    const harness=await createHarness();
    running.push(harness);
    const response=await post(harness,"tools/call",{
      name:"read_project_summary",
      arguments:{},
    },{name:"read_project_summary"});
    expect(response.status).toBe(200);
    const payload=await response.json() as {result:{structuredContent:{projectId:string;toolId:string};isError:boolean}};
  ...
  it("rejects unknown tools and oversized authenticated requests",async()=>{
    const harness=await createHarness();
    running.push(harness);
    const unknown=await post(harness,"tools/call",{name:"delete_project",arguments:{}},{name:"delete_project"});
  ...
  it("never exposes credential secrets and marks inactive clients stale",async()=>{
    ...
    expect(harness.controller.getSnapshot(lastSeen+LOCAL_MCP_STALE_CLIENT_MS+1).clients[0]?.status).toBe("disconnected");
  });
});