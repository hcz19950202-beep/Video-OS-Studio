...
const createMission=async(request:APIRequestContext,projectId:string,title:string)=>{
  const response=await request.post(`/api/projects/${encodeURIComponent(projectId)}/missions`,{data:{
    title,
...
test("B7 Campaign dashboard reloads durable truth and isolates pending Mission cancellation",async({page,request})=>{
  ...
  const create=await request.post("/api/campaigns",{data:{
    title:`B7 Browser Campaign ${suffix}`,
    maxConcurrency:2,
    sharedReferences:{assetIds:["asset.browser-proof"],policyIds:[],skillIds:[],exportTemplateIds:[]},
    missions:[
  ...
  await expect(page.getByRole("heading",{name:`B7 Browser Campaign ${suffix}`})).toBeVisible();
