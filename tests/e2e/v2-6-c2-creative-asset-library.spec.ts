...
test("V2.6 C2 browses, searches and inspects Creative Assets in the Studio Library",async({page})=>{
  ...
  await page.route("**/api/creative-assets**",async route=>{
    ...
    const items=fixtures.filter(item=>{
      const searchable=`${item.name} ${item.tags.join(" ")} ${searchTextById.get(item.id)??""}`.toLocaleLowerCase();
