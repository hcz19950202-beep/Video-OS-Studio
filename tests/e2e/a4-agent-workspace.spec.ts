...
const applyCommand=async(page:Page,projectId:string,commandId:string,command:ProjectCommand):Promise<Project>=>{
  ...
  return page.evaluate(async({id,expectedRevision,operationId,payload})=>{
    const response=await fetch(`/api/projects/${encodeURIComponent(id)}/commands`,{
      method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({expectedRevision,commandId:operationId,command:payload}),
    });
...
const motionClipIds=(project:Project)=>project.tracks.find(track=>track.id==="motion-main")?.clips.filter(clip=>clip.type==="motion").map(clip=>clip.id)??[];
...
const openAgent=async(page:Page)=>{
  ...
  await expect(page.getByRole("tab",{name:"Composer",exact:true})).toHaveAttribute("aria-selected","true");
...
test("A4 Agent selection → proposal → Review/Apply → reopen → stale guard",async({page})=>{
  ...
  await applyCommand(page,projectId,"a4-seed-scene",{type:"add-scene",scene:{id:SCENE_ID,name:"Proof",semanticType:"proof",startFrame:0,endFrame:createdBody.project.canvas.durationInFrames,visualStrategy:{intensity:"high",preferredEngines:["remotion"]}}});
  await applyCommand(page,projectId,"a4-seed-caption",{type:"add-clip",trackId:"captions-main",clip:{id:CAPTION_ID,type:"caption",text:"90% complete in 15 days — send us your project",startFrame:30,durationInFrames:90,enabled:true,layer:100,preset:"primary",emphasis:"numbers",keywords:[]}});
  ...
  await sendAgent(page,"Create a reviewable visual proposal for the selected proof caption. Do not apply it.");
  ...
  await expect.poll(async()=>motionClipIds(await readProject(page,projectId)).includes(appliedMotionId)).toBe(false);

  await page.reload();
  await openRecentProject(page,projectId);
  await openAgent(page);
  await expect(page.locator(".a4-agent-conversation")).toContainText("Create a reviewable visual proposal for the selected proof caption.");
