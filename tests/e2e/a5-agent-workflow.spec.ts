...
const listProjectWorkflows=async(page:Page,projectId:string):Promise<WorkflowRun[]>=>page.evaluate(async id=>{
  const response=await fetch(`/api/workflows?projectId=${encodeURIComponent(id)}`,{cache:"no-store"});
...
test("A5 Agent Workflow proposal stays inert until Review + explicit Confirm",async({page})=>{
  ...
  await page.locator(".a4-agent-composer textarea").fill("Create a first draft workflow proposal for this product ad using the current source asset. Do not execute the Workflow until I explicitly confirm it.");
  ...
  await expect.poll(async()=>(await listProjectWorkflows(page,projectId)).length,{timeout:20_000}).toBe(1);
