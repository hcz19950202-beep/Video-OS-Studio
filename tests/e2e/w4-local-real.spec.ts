...
const enabled=process.env.W4_WINDOWS_WORKFLOW_UI_SMOKE==="1"&&Boolean(process.env.W4_SOURCE_VIDEO);

test.skip(!enabled,"Set W4_WINDOWS_WORKFLOW_UI_SMOKE=1 and W4_SOURCE_VIDEO to run the real W4 Windows acceptance.");
...
test("W4 real browser Generate First Draft review edit approve and final render",async({page},testInfo)=>{
  ...
  const importResponse=page.waitForResponse(response=>response.request().method()==="POST"&&response.url().includes(`/api/projects/${encodeURIComponent(projectId)}/media?`));
  await page.locator('input[type="file"]').setInputFiles(source);expect((await importResponse).ok()).toBeTruthy();
  await expect.poll(async()=>page.evaluate(async id=>{const response=await fetch(`/api/projects/${encodeURIComponent(id)}`,{cache:"no-store"});const body=await response.json() as {project:Project};return body.project.assets.filter(asset=>asset.kind==="video").length;},projectId),{timeout:120_000}).toBeGreaterThan(0);

  await page.getByTitle("AI").click();await page.getByRole("tab",{name:"Workflow",exact:true}).click();
  const startResponse=page.waitForResponse(response=>response.request().method()==="POST"&&response.url().endsWith("/api/workflows"));
  await page.getByRole("button",{name:"Generate First Draft",exact:true}).click();const workflowCreated=await startResponse;expect(workflowCreated.status()).toBe(201);const createdWorkflow=await workflowCreated.json() as {workflow:WorkflowRun};const workflowId=createdWorkflow.workflow.id;

  await expect(page.locator('.v22-review-card[data-review-stage="CONTENT_REVIEW"]')).toBeVisible({timeout:5*60_000});
  const beforeEdit=await page.evaluate(async id=>{const response=await fetch(`/api/projects/${encodeURIComponent(id)}`,{cache:"no-store"});return(await response.json() as {project:Project}).project;},projectId);
  const caption=page.locator('[data-clip-id^="wf-caption-"]').first();await expect(caption).toBeVisible();await caption.click();
  const fontSize=page.getByLabel("Font Size");await expect(fontSize).toBeVisible();await fontSize.fill("60");await fontSize.press("Enter");
  await expect.poll(async()=>page.evaluate(async({id,revision})=>{const response=await fetch(`/api/projects/${encodeURIComponent(id)}`,{cache:"no-store"});return(await response.json() as {project:Project}).project.project.revision>revision;},{id:projectId,revision:beforeEdit.project.revision})).toBe(true);

  await page.getByRole("button",{name:"Approve & Continue",exact:true}).click();
  await expect(page.locator('.v22-review-card[data-review-stage="ASSEMBLY_REVIEW"]')).toBeVisible({timeout:6*60_000});
  await page.getByRole("button",{name:"Approve & Continue",exact:true}).click();
  await expect(page.locator('.v22-workflow-panel[data-workflow-state="completed"]')).toBeVisible({timeout:8*60_000});

  const finalRun=await page.evaluate(async id=>{const response=await fetch(`/api/workflows/${encodeURIComponent(id)}`,{cache:"no-store"});return(await response.json() as {workflow:WorkflowRun}).workflow;},workflowId);
