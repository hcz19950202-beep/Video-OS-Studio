...
test("C4 Connection Center exposes only authenticated local read connectivity",async({page})=>{
  ...
  for(const tool of READ_TOOLS)await expect(catalog.getByText(tool,{exact:true})).toBeVisible();
  ...
  await page.getByTestId("connection-center").getByRole("button",{name:"Stop bridge",exact:true}).click();
  await expect(page.getByTestId("mcp-bridge-status")).toHaveText("stopped");
});