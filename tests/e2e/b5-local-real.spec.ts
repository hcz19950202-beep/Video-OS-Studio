...
test("B5 real Windows Mission checkpoint restart retry cancellation and workspace durability", async ({
  page,
}) => {
  ...
  await page
    ...
    .fill("Prove controlled Mission execution over this real imported video without producing B6 output.");
  ...
  const { stdout: probeOutput } = await execFileAsync(
    ffprobe,
    ["-v", "error", "-show_entries", "format=duration:stream=codec_name", "-of", "json", assetPath],
    { windowsHide: true },
  );
  ...
  expect(afterRetry.steps.find((step) => step.stepId === "cancelled-before-job")?.status).toBe(
    "pending",
  );
