...
const openRecentProject=async(page:import("@playwright/test").Page,projectId:string)=>{
  ...
  const response=page.waitForResponse(item=>item.request().method()==="GET"&&item.url().endsWith(`/api/projects/${encodeURIComponent(projectId)}`));
  await recent.click();expect((await response).ok()).toBeTruthy();
  await expect(page.locator(".v21-project-title")).toContainText(PROJECT_NAME);
};

const openMissionWorkspace=async(page:import("@playwright/test").Page)=>{
...
test("B5c Mission workspace reloads durable Mission truth and exposes no fake executor action",async({page})=>{
  ...
  const createProjectResponse=page.waitForResponse(response=>response.request().method()==="POST"&&response.url().endsWith("/api/projects"));
  await page.getByRole("button",{name:"Create Project",exact:true}).click();
  const created=await createProjectResponse;expect(created.status()).toBe(201);
  const projectId=((await created.json()) as {project:{project:{id:string}}}).project.project.id;
  ...
  await page.getByLabel("Production brief").fill("Create a concise social video that leads with measurable proof and keeps final review enabled.");
  const createMissionResponse=page.waitForResponse(response=>response.request().method()==="POST"&&response.url().endsWith(`/api/projects/${encodeURIComponent(projectId)}/missions`));
  ...
  await expect(page.getByRole("button",{name:/^(Run|Advance|Approve|Approve and continue)$/i})).toHaveCount(0);

  const autonomy=page.getByLabel("Autonomy mode");
  await expect(autonomy).toHaveValue("guided");
  const patchResponse=page.waitForResponse(response=>response.request().method()==="PATCH"&&response.url().includes(`/api/projects/${encodeURIComponent(projectId)}/missions/`));
  ...
  await expect(page.getByText(/Derived from durable render, QA, and revision evidence/)).toBeVisible();
});
