...
test("C2 opens directly into one Agent conversation with execution policy and non-disruptive advanced details", async ({
  page,
}) => {
  ...
  await expect(page.getByTestId("agent-conversation-list")).toContainText(
    "You do not need to choose Mission, Agent, Composer, or Workflow first",
  );
  ...
  const executionMode = page.getByRole("combobox", {
    name: "Execution mode",
    exact: true,
  });
  ...
  await expect(page.getByTestId("agent-execution-mode")).toContainText(
    "Read, analyze, search, plan and propose only",
  );

  await page.getByText("Advanced", { exact: true }).click();
  await expect(
    page.getByRole("button", { name: "Mission", exact: true }),
  ).toBeVisible();
  await expect(
    page.getByRole("button", { name: "Composer", exact: true }),
  ).toBeVisible();
  await expect(
