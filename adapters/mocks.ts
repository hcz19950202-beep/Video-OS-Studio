...
export class MockFfmpegAdapter implements FfmpegAdapter{constructor(private readonly result:MediaProbeResult={durationSeconds:3,hasAudio:true},private readonly peaks:number[]=[.1,.4,.8,.3]){}async probe(){return this.result;}async waveformPeaks(_inputPath:string,points:number){return Array.from({length:points},(_,index)=>this.peaks[index%this.peaks.length]??0);}}
