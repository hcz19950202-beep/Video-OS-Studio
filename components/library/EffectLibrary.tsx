...
export const EffectLibrary=({project,onCommand,onProjectChange,mode="sidebar"}:{project:Project;onCommand:(command:ProjectCommand,message:string)=>Promise<void>;onProjectChange:(project:Project)=>void;mode?:LibraryMode})=>{
  ...
  const addEffect=(effect:typeof EFFECT_REGISTRY[number])=>{
    const duration=Math.min(effect.defaultDurationInFrames,Math.max(1,project.canvas.durationInFrames-frame));
    void onCommand({type:"add-clip",trackId:"motion-main",clip:{id:`motion-${effect.id}-${Date.now()}`,type:"motion",engine:"remotion",effectId:effect.id,props:effect.defaults,startFrame:frame,durationInFrames:duration,enabled:true,layer:10}},`${translateEffectName(locale,effect.id,effect.name)} · ${t("status.effectAdded")}`);
  };
  ...
  return <div className={`effect-library effect-library-${mode}`}>
    ...
    <div className={`effect-list ${mode==="catalog"?"catalog-grid":""}`}>{filtered.length?filtered.map(effect=><button className="effect-card" key={effect.id} onClick={()=>addEffect(effect)} title={t("library.add")}><div className="effect-thumb"><img alt="" src={effect.thumbnail}/><span>＋</span></div><span><strong>{translateEffectName(locale,effect.id,effect.name)}</strong><small>{categoryLabel(effect.category)} · {effect.defaultDurationInFrames}f</small></span></button>):<p className="effect-empty">{t("library.empty")}</p>}</div>
