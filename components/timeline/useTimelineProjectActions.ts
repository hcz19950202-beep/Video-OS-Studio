...
export const useTimelineProjectActions=(project:Project)=>{const setProject=useProjectStore(state=>state.setProject);const pushHistory=useHistoryStore(state=>state.push);const takeUndo=useHistoryStore(state=>state.takeUndo);const takeRedo=useHistoryStore(state=>state.takeRedo);
  ...
  const undo=async(labelPrefix="Undo")=>{const entry=takeUndo(project.project.id);if(!entry)return;await postCommand({type:"restore-project-snapshot",snapshot:entry.before},`${labelPrefix}: ${entry.label}`,{record:false});};
  const redo=async(labelPrefix="Redo")=>{const entry=takeRedo(project.project.id);if(!entry)return;await postCommand({type:"restore-project-snapshot",snapshot:entry.after},`${labelPrefix}: ${entry.label}`,{record:false});};
