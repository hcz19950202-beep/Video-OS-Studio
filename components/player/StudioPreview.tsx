...
import {usePlayerStoreBridge} from "@/components/player/usePlayerStoreBridge";
...
const PlayerTimeReadout=({fps,end,label}:{fps:number;end:number;label:string})=>{const currentFrame=usePlayerStore(state=>state.currentFrame);return <div className="time-readout"><strong>{formatStudioTime(currentFrame,fps)}</strong><span>/ {formatStudioTime(end,fps)} · {label} {currentFrame}/{end}</span></div>;};
