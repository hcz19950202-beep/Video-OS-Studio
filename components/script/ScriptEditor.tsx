...
export const ScriptEditor=({project,onProjectChange,onCommand}:Props)=>{
  ...
  const currentFrame=usePlayerStore(state=>state.currentFrame);const requestSeek=usePlayerStore(state=>state.requestSeek);const selectScriptRange=useSelectionStore(state=>state.selectScriptRange);
  ...
  const sourceFrame=mapTimelineFrameToSourceFrame(project,currentFrame);const blocking=project.scenes.length>0||project.tracks.some(track=>track.type!=="video"&&track.clips.length>0);
  ...
  return <section className="script-editor">
    ...
    <div className="script-segments">{segments.map((segment,index)=>{const removed=segment.status==="removed";const sourceRange=getSegmentSourceRange(segment);const timelineStart=sourceRange?mapSourceFrameToTimelineFrame(project,sourceRange.startFrame):null;const scene=segment.sceneId?project.scenes.find(item=>item.id===segment.sceneId):undefined;const sceneLabel=scene?SCENE_LABELS[scene.semanticType]:undefined;return <article key={segment.id} className={`script-segment ${removed?"is-removed":""}`}>
      ...
      <p className="script-words" aria-label={segmentText(segment)}>{segment.words.map(word=>{const active=sourceFrame!==null&&sourceFrame>=word.startFrame&&sourceFrame<word.endFrame;const seek=mapSourceFrameToTimelineFrame(project,word.startFrame);return <button key={word.id} className={active?"current-word":""} disabled={removed||seek===null} onClick={()=>{selectScriptRange({startWordId:word.id,endWordId:word.id});if(seek!==null)requestSeek(seek);}}>{word.text}</button>;})}</p>
