...
export const AgentNativeContextDock=({inspector,tab,onTabChange}:Props)=>{
  ...
  return <section className={styles.contextDock} data-testid="agent-context-dock">
    ...
    <div className={styles.contextBody} data-context-tab={tab}>
      ...
      {tab==="qa"?<div className={styles.qaSummary}>{project?<><div><strong>{project.assets.length}</strong><span>{zh?"素材":"Assets"}</span></div><div><strong>{enabledClips}</strong><span>{zh?"启用片段":"Enabled clips"}</span></div><div><strong>{captionClips.length}</strong><span>{zh?"字幕片段":"Caption clips"}</span></div><p>{zh?"项目级健康视图；完整 QA finding 上下文从统一 Agent QA 卡片附加。":"Project health view; attach exact QA finding context from the unified Agent QA card."}</p></>:<p>{zh?"打开项目后查看质检上下文。":"Open a project to view QA context."}</p>}</div>:null}
