...
type Props={topbar:ReactNode;rail:ReactNode;content:ReactNode;viewer:ReactNode;inspector:ReactNode;timeline:ReactNode;onOpenProjects:()=>void;onOpenCampaigns:()=>void;preferredMissionId?:string};

export const ResizableWorkspaceShell=({topbar,rail,content,viewer,inspector,timeline,onOpenProjects,onOpenCampaigns,preferredMissionId}:Props)=>{
  const{layout,setLayout,toggleLeft,toggleInspector,toggleTimeline}=useWorkspaceLayout();
  const[drag,setDrag]=useState<ResizeState>(null);const[draft,setDraft]=useState<WorkspaceLayout|null>(null);const[leftSurface,setLeftSurface]=useState<AgentNativeSurface>("agent");const[contextTab,setContextTab]=useState<AgentNativeContextTab>(preferredMissionId?"mission":"inspector");const display=draft??layout;
  ...
  const openProjects=()=>{setLeftSurface("tools");onOpenProjects();};
  const openHistory=()=>{setContextTab("history");if(layout.inspectorCollapsed)toggleInspector();};
  ...
  return <main className="v21-workspace" style={style} data-workspace-version="2.5-c1">
    {topbar}
    <AgentNativeCommandStrip onOpenProjects={openProjects} onOpenCampaigns={onOpenCampaigns} onOpenHistory={openHistory}/>
    <section className="v21-workspace-body"><div className="v21-upper-workspace">
      ...
      {!display.inspectorCollapsed?<aside className="v21-inspector-panel" data-workspace-region="context"><AgentNativeContextDock inspector={inspector} tab={contextTab} onTabChange={setContextTab} preferredMissionId={preferredMissionId}/></aside>:null}
    </div>
