...
export const InspectorRegistryShell=({project,children}:{project:Project;children:ReactNode})=>{
  ...
  useEffect(()=>setActive(0),[context]);
  const go=(tab:Tab,index:number)=>{setActive(index);const root=rootRef.current;if(!root)return;const target=tab.selector?root.querySelector<HTMLElement>(tab.selector):root.querySelectorAll<HTMLElement>(".inspector-section")[tab.sectionIndex??0];target?.scrollIntoView({block:"start",behavior:"smooth"});target?.focus?.({preventScroll:true});};
