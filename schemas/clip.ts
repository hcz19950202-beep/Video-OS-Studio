...
export const MotionTransformSchema=z.object({x:z.number().finite().default(0),y:z.number().finite().default(0),scale:z.number().min(0.1).max(5).default(1),opacity:z.number().min(0).max(1).default(1),anchor:MotionAnchorSchema.default("center")});
...
export const BrollClipSchema=z.object({...BaseClipShape,type:z.literal("broll"),assetId:z.string().min(1),fit:z.enum(["cover","contain"]).optional(),muted:z.boolean().optional(),volume:z.number().min(0).max(2).optional(),fadeInFrames:z.number().int().nonnegative().optional(),fadeOutFrames:z.number().int().nonnegative().optional(),transform:MotionTransformSchema.optional(),...FrameTimingSchema.shape});
