...
export class AssetLibraryService{
  ...
  async update(presetId:string,patch:{favorite?:boolean;status?:"draft"|"production-ready"}):Promise<AssetPreset>{
    const registry=await this.load();const index=registry.presets.findIndex((preset)=>preset.id===presetId);if(index<0)throw new Error(`Preset ${presetId} not found.`);
    const next=AssetPresetSchema.parse({...registry.presets[index],...patch,updatedAt:new Date().toISOString()});registry.presets[index]=next;await this.save(registry);
