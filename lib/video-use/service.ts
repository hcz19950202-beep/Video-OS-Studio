...
export class VideoUseService{
  ...
  private async baseline(projectId:string,meta?:MutationMeta){
    const project=await this.repository.load(projectId);
    if(meta&&project.project.revision!==meta.expectedRevision)throw new ProjectRevisionConflictError(meta.expectedRevision,project.project.revision);
    return project;
  }

  async prepare(projectId:string,meta?:MutationMeta){
    ...
    const committed=await this.mutations.mutate({
      projectId,expectedRevision,operationId,kind:"video-use",
      payload:{action:"prepare",assetId:asset.id,script},
      apply:current=>applyProjectCommandTransaction(current,{id:operationId,label:"video-use · Prepare transcript",commands:[{type:"set-script-document",script}]}),
    });
    return{
  ...
  async applyEdl(projectId:string,edlInput:VideoUseEdl,meta?:MutationMeta):Promise<Project>{
    const baseline=await this.baseline(projectId,meta);
    const expectedRevision=meta?.expectedRevision??baseline.project.revision;
    const operationId=meta?.operationId??`video-use-edl-${randomUUID()}`;
    const edl=VideoUseEdlSchema.parse(edlInput);
    const asset=this.primaryVideo(baseline);
    const maxFrames=asset.durationInFrames??baseline.canvas.durationInFrames;

    const commands:ProjectCommand[]=[];
    const blocked=baseline.tracks.filter(track=>["motion","broll","audio"].includes(track.type)&&track.clips.length>0);
    ...
    if(baseline.scenes.length)throw new Error("Apply the confirmed EDL before Scene design so Scene timing is not silently corrupted.");
    if(baseline.script.segments.some(segment=>segment.status==="removed"))throw new Error("Do not apply a new EDL after Script cuts. Restore the Script baseline or start from a fresh project first.");

    const sourceCaptions=(baseline.tracks.find(track=>track.id==="captions-main")?.clips??[]).filter((clip):clip is CaptionClip=>clip.type==="caption");
    const videoTrack=baseline.tracks.find(track=>track.id==="video-main");
    for(const clip of videoTrack?.clips??[])commands.push({type:"remove-clip",clipId:clip.id});
    for(const clip of sourceCaptions)commands.push({type:"remove-clip",clipId:clip.id});
    ...
    edl.ranges.forEach((range,index)=>{
      const sourceStart=secondsToFrames(range.start,baseline.canvas.fps);
      const sourceEnd=Math.max(sourceStart+1,secondsToFrames(range.end,baseline.canvas.fps));
      ...
      commands.push({type:"add-clip",trackId:"video-main",clip:{id:`edl-video-${index+1}`,type:"video",assetId:asset.id,startFrame:outputCursor,durationInFrames,sourceStartFrame:sourceStart,volume:1,enabled:true,layer:0}});
    ...
    for(const caption of remappedCaptions)commands.push({type:"add-clip",trackId:"captions-main",clip:caption});
    commands.push({type:"set-duration",durationInFrames:Math.max(1,outputCursor)});

    if(baseline.script.segments.length){
      const ranges=mergeSourceRanges(baseSourceRanges);
      const script=structuredClone(baseline.script);
    ...
    const committed=await this.mutations.mutate({
      projectId,expectedRevision,operationId,kind:"video-use",payload:{action:"apply-edl",edl},
      apply:current=>applyProjectCommandTransaction(current,{id:operationId,label:"video-use · Apply EDL",commands}),
    });
