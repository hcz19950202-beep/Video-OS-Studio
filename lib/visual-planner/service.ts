...
import {VisualPlanSchema,type VisualPlan,type VisualPlanDiff} from "@/lib/visual-planner/schema";
  ...
  async apply(projectId:string,planInput:VisualPlan,selectedIds:string[]):Promise<VisualPlanApplyResult>{
    ...
    for(const suggestion of suggestions){
      ...
      if(recommendation.engine==="remotion"){
        ...
        commands.push({type:"add-clip",trackId:"motion-main",clip:{id:clipId,type:"motion",engine:"remotion",effectId:recommendation.effectId,props,startFrame:suggestion.startFrame,durationInFrames,enabled:true,layer:10}});
        continue;
      }
      ...
      if(recommendation.engine==="hyperframes"){
        ...
        commands.push({type:"add-clip",trackId:"motion-main",clip:{id:clipId,type:"motion",engine:"hyperframes",effectId:prepared.effectId,assetId:prepared.assetId,props:prepared.props,startFrame:suggestion.startFrame,durationInFrames,enabled:true,layer:20}});
        continue;
      }
      ...
      if(recommendation.engine==="broll"){
        ...
        commands.push({type:"add-clip",trackId:"broll-main",clip:{id:clipId,type:"broll",assetId,startFrame:suggestion.startFrame,durationInFrames,sourceStartFrame:0,enabled:true,layer:5,fit:"cover",muted:true}});
      }
    }
