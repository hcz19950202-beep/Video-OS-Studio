...
const candidateFor=(project:Project,scene:Scene,text:string):Candidate|null=>{
  ...
  if(percent){const number=Number(percent[1]);return{semanticType:"percentage",preferredDuration:105,recommendation:{engine:"remotion",effectId:"metric-focus",props:{title:"KEY METRIC",value:percent[1],unit:percent[2].replace("％","%"),accentColor:accent,progress:Math.max(0,Math.min(100,number))}},reason:`The spoken line contains percentage proof inside ${scene.name}; percentage evidence is a high-value visual moment.`,confidence:.95,alternatives:[{engine:"remotion",effectId:"big-number",reason:"Use a simpler numeric card if progress framing is not needed."},{engine:"none",reason:"Leave the proof spoken-only if the surrounding scene is already visually dense."}]};}
  if(quantity)return{semanticType:"number",preferredDuration:90,recommendation:{engine:"remotion",effectId:"big-number",props:{label:"KEY NUMBER",value:quantity[1],suffix:suffixLabel(quantity[2]),accentColor:accent,fontSize:180,animationStyle:"scale"}},reason:`The line contains a concrete time/count quantity in ${scene.name}, making it strong evidence worth isolating.`,confidence:.93,alternatives:[{engine:"remotion",effectId:"metric-focus",reason:"Use a metric treatment when the number represents measured performance."},{engine:"none",reason:"Keep it verbal when another visual event is already carrying the same point."}]};
