...
import {WorkflowArtifactReferenceSchema,WorkflowRunSchema,type WorkflowDefinition,type WorkflowError,type WorkflowRun,type WorkflowStageDefinition,type WorkflowStageExecution} from "@/lib/workflows/schema";
...
export class WorkflowRunner{
  ...
  private async requireRun(workflowId:string){
    const run=await this.store.get(workflowId);
    if(!run)throw new WorkflowNotFoundError(workflowId);
    return run;
  }
  ...
  private async activity(workflowId:string,event:WorkflowActivityEvent,details:Partial<{stageId:string;jobId:string;data:Record<string,unknown>}>= {}){
    const record=WorkflowActivitySchema.parse({
      id:randomUUID(),workflowId,at:nowIso(),event,
      stageId:details.stageId,
      jobId:details.jobId,
      details:details.data,
    });
    await this.store.appendActivity(record);
    return record;
  }
  ...
  async waitForIdle(workflowId:string){
    for(;;){
      const loop=this.activeLoops.get(workflowId);
      if(!loop)return;
      await loop;
    }
  }
  ...
  async start(workflowId:string){
    const run=await this.withRunLock(workflowId,async()=>{
      ...
      const updated=WorkflowRunSchema.parse({...current,status:"running",updatedAt:nowIso(),error:undefined});
      return this.store.save(updated);
    });
    await this.activity(workflowId,"workflow-started");
  ...
  async pause(workflowId:string){
    ...
    await this.activity(workflowId,"workflow-paused");
    return run;
  }
  ...
  async resume(workflowId:string){
    ...
    await this.activity(workflowId,"workflow-resumed");
    this.schedule(workflowId);
    return run;
  }
  ...
  async cancel(workflowId:string){
    ...
    const run=await this.withRunLock(workflowId,async()=>{
      ...
      if(current.currentStageId){
        const execution=getExecution(current,current.currentStageId);
        jobIds.push(...execution.jobIds);
        if(["pending","ready","running","waiting_review","failed","interrupted"].includes(execution.status)){
          ...
          stageExecutions=replaceExecution(current,WorkflowRunSchema.shape.stageExecutions.element.parse({...execution,status:"cancelled",completedAt:nowIso()}));
        }
      }
    ...
    await this.activity(workflowId,"workflow-cancelled");
    return run;
  }
  ...
  async retryStage(workflowId:string,stageId:string){
    const run=await this.withRunLock(workflowId,async()=>{
      ...
      const definition=this.definitionFor(current);
      ...
      assertWorkflowStageStatusTransition(execution.status,"ready");
      assertWorkflowRunStatusTransition(current.status,"running");
      const retried=WorkflowRunSchema.shape.stageExecutions.element.parse({...execution,status:"ready",attemptId:undefined,startedAt:undefined,completedAt:undefined,baseProjectRevision:undefined,inputDigest:undefined,outputDigest:undefined,artifactIds:[],error:undefined});
    ...
    await this.activity(workflowId,"stage-retried",{stageId});
    this.schedule(workflowId);
    return run;
  }
  ...
  async approveCheckpoint(workflowId:string,checkpointId:string,projectRevision?:number){
    const run=await this.withRunLock(workflowId,async()=>{
      ...
      assertWorkflowStageStatusTransition(execution.status,"completed");
      assertWorkflowRunStatusTransition(current.status,"running");
      const at=nowIso();
      const revision=projectRevision===undefined?current.lastKnownProjectRevision:Math.max(current.lastKnownProjectRevision,projectRevision);
      const completedExecution=WorkflowRunSchema.shape.stageExecutions.element.parse({...execution,status:"completed",completedAt:at,error:undefined});
    ...
    await this.activity(workflowId,"review-approved",{stageId:run.stageExecutions.find(item=>item.status==="completed"&&run.checkpoints.some(checkpoint=>checkpoint.id===checkpointId&&checkpoint.stageId===item.stageId))?.stageId,data:{checkpointId}});
    this.schedule(workflowId);
    return run;
  }
  ...
  async recover(){
    ...
    for(const run of runs){
      ...
      if(active?.status==="running"&&active.jobIds.length===0){
        await this.markInterrupted(run.id,active.stageId,{code:"WORKFLOW_STAGE_INTERRUPTED",message:"The Video OS process stopped while a non-job workflow stage was active.",retryable:true});
        continue;
      }
  ...
  private async runLoop(workflowId:string){
    for(;;){
      let run=await this.requireRun(workflowId);
      if(run.status!=="running")return;
      const definition=this.definitionFor(run);
      const active=run.stageExecutions.find(item=>item.status==="running");
      if(active){
        if(active.jobIds.length===0)return;
        await this.reconcileJobStage(run,definition,active);
        continue;
      }

      run=await this.markReadyStages(run,definition);
      if(run.status!=="running")return;
      if(run.stageExecutions.every(item=>item.status==="completed"||item.status==="skipped")){
        await this.completeWorkflow(run.id);
        return;
      }

      const readyStage=definition.stages.find(stage=>getExecution(run,stage.id).status==="ready");
      if(!readyStage)return;
      if(readyStage.kind==="checkpoint"){
        await this.enterCheckpoint(run,readyStage);
        return;
      }
  ...
  private async markReadyStages(run:WorkflowRun,definition:WorkflowDefinition){
    ...
    const updated=await this.withRunLock(run.id,async()=>{
      ...
      for(const stage of definition.stages){
        const execution=stageExecutions.find(item=>item.stageId===stage.id);
        if(!execution||execution.status!=="pending")continue;
        const dependenciesReady=stage.dependsOn.every(dependencyId=>{
          const dependency=stageExecutions.find(item=>item.stageId===dependencyId);
          return dependency!==undefined&&completedDependencyStatuses.has(dependency.status);
        });
        if(!dependenciesReady)continue;
        assertWorkflowStageStatusTransition(execution.status,"ready");
        const ready=WorkflowRunSchema.shape.stageExecutions.element.parse({...execution,status:"ready"});
        stageExecutions=stageExecutions.map(item=>item.stageId===stage.id?ready:item);
        readyStageIds.push(stage.id);
      }
    ...
    for(const stageId of readyStageIds)await this.activity(run.id,"stage-ready",{stageId});
    return updated;
  }

  private async executeStage(run:WorkflowRun,definition:WorkflowDefinition,stage:WorkflowStageDefinition){
    const attemptId=randomUUID();
    const operationId=`workflow:${run.id}:stage:${stage.id}:attempt:${attemptId}`;
    const started=await this.withRunLock(run.id,async()=>{
      const current=await this.requireRun(run.id);
      if(current.status!=="running")return null;
      const execution=getExecution(current,stage.id);
      if(execution.status!=="ready")return null;
      assertWorkflowStageStatusTransition(execution.status,"running");
      const previousJobIds=[...execution.jobIds];
      const running=WorkflowRunSchema.shape.stageExecutions.element.parse({...execution,status:"running",attempt:execution.attempt+1,attemptId,startedAt:nowIso(),completedAt:undefined,baseProjectRevision:current.lastKnownProjectRevision,outputDigest:undefined,jobIds:[],artifactIds:[],operationIds:[...execution.operationIds,operationId],error:undefined});
    ...
    if(!started)return;
    await this.activity(run.id,"stage-started",{stageId:stage.id,data:{attempt:started.execution.attempt,attemptId,operationId}});

    const executor=this.stages.get(stage.executorKey);
    const context:WorkflowStageExecutionContext={run:started.run,definition,stage,execution:started.execution,attemptId,operationId,previousJobIds:started.previousJobIds};
    try{
      const result=await executor.start(context);
      if(result.kind==="completed"){
        await this.completeStage(run.id,stage.id,result);
        return;
      }
      const jobId=JobIdSchema.parse(result.jobId);
      await this.attachJob(run.id,stage.id,jobId);
      await this.reconcileJobStage(await this.requireRun(run.id),definition,getExecution(await this.requireRun(run.id),stage.id));
    }catch(error){await this.failStage(run.id,stage,error);}
  }

  private async attachJob(workflowId:string,stageId:string,jobId:string){
    await this.withRunLock(workflowId,async()=>{
      const current=await this.requireRun(workflowId);
      const execution=getExecution(current,stageId);
      if(current.status!=="running"||execution.status!=="running")return;
      const attached=WorkflowRunSchema.shape.stageExecutions.element.parse({...execution,jobIds:[jobId]});
      await this.store.save(WorkflowRunSchema.parse({...current,stageExecutions:replaceExecution(current,attached),updatedAt:nowIso()}));
    });
    await this.activity(workflowId,"job-attached",{stageId,jobId});
  }
  ...
  private async reconcileJobStage(run:WorkflowRun,definition:WorkflowDefinition,execution:WorkflowStageExecution){
    if(!this.jobs){
      ...
      if(stage)await this.failStage(run.id,stage,{code:"WORKFLOW_JOB_RUNTIME_UNAVAILABLE",message:"Workflow job runtime is unavailable.",retryable:true});
      return;
    }
    const jobId=execution.jobIds[execution.jobIds.length-1];
    if(!jobId)return;
    ...
    for(;;){
      ...
      if(!job){
        const stage=definition.stages.find(item=>item.id===execution.stageId);
        if(stage)await this.failStage(run.id,stage,{code:"WORKFLOW_JOB_NOT_FOUND",message:`Workflow job ${jobId} was not found.`,retryable:true});
        return;
      }
      if(isTerminalJobStatus(job.status))break;
      await sleep(this.jobPollIntervalMs);
    }
    ...
    if(job.status==="completed"){
      try{
        ...
        const currentExecution=getExecution(latest,stage.id);
        const attemptId=currentExecution.attemptId;
        const operationId=currentExecution.operationIds[currentExecution.operationIds.length-1];
        ...
        const completion=executor.reconcileJob?await executor.reconcileJob({run:latest,definition,stage,execution:currentExecution,attemptId,operationId,previousJobIds:[]},job):{};
        await this.completeStage(run.id,stage.id,completion);
      }catch(error){await this.failStage(run.id,stage,error);}
      return;
    }
    if(job.status==="interrupted"){
      await this.markInterrupted(run.id,stage.id,{code:job.error?.code??"WORKFLOW_JOB_INTERRUPTED",message:job.error?.message??`Workflow job ${jobId} was interrupted.`,retryable:stage.retryable&&job.error?.retryable!==false,details:{jobId}});
      return;
    }
  ...
  private async completeStage(workflowId:string,stageId:string,completion:WorkflowStageCompletion){
    ...
    const completed=await this.withRunLock(workflowId,async()=>{
      const current=await this.requireRun(workflowId);
      const execution=getExecution(current,stageId);
      if(execution.status!=="running")return current;
      assertWorkflowStageStatusTransition(execution.status,"completed");
      const at=nowIso();
      const done=WorkflowRunSchema.shape.stageExecutions.element.parse({...execution,status:"completed",completedAt:at,outputDigest:completion.outputDigest,artifactIds:parsedArtifacts.map(item=>item.id),error:undefined});
      const artifactIds=new Set(parsedArtifacts.map(item=>item.id));
      const artifacts=[...current.artifacts.filter(item=>!artifactIds.has(item.id)),...parsedArtifacts];
    ...
    if(getExecution(completed,stageId).status==="completed")await this.activity(workflowId,"stage-completed",{stageId});
    return completed;
  }

  private async failStage(workflowId:string,stage:WorkflowStageDefinition,error:unknown){
    const normalized=normalizeError(error,stage.retryable);
    const failed=await this.withRunLock(workflowId,async()=>{
      const current=await this.requireRun(workflowId);
      if(current.status==="cancelled"||current.status==="completed")return current;
      const execution=getExecution(current,stage.id);
      if(execution.status!=="running")return current;
      assertWorkflowStageStatusTransition(execution.status,"failed");
      if(current.status==="running")assertWorkflowRunStatusTransition(current.status,"failed");
      const at=nowIso();
      const failedExecution=WorkflowRunSchema.shape.stageExecutions.element.parse({...execution,status:"failed",completedAt:at,error:normalized});
    ...
    if(failed.status==="failed"){
      await this.activity(workflowId,"stage-failed",{stageId:stage.id,data:{code:normalized.code,retryable:normalized.retryable}});
      await this.activity(workflowId,"workflow-failed",{stageId:stage.id,data:{code:normalized.code}});
    }
    return failed;
  }

  private async markInterrupted(workflowId:string,stageId:string,error:WorkflowError){
    const interrupted=await this.withRunLock(workflowId,async()=>{
      const current=await this.requireRun(workflowId);
      const execution=getExecution(current,stageId);
      if(execution.status!=="running")return current;
      assertWorkflowStageStatusTransition(execution.status,"interrupted");
      if(current.status==="running")assertWorkflowRunStatusTransition(current.status,"interrupted");
      const at=nowIso();
      const interruptedExecution=WorkflowRunSchema.shape.stageExecutions.element.parse({...execution,status:"interrupted",completedAt:at,error});
    ...
    if(interrupted.status==="interrupted"){
      await this.activity(workflowId,"stage-interrupted",{stageId,data:{code:error.code}});
      await this.activity(workflowId,"workflow-interrupted",{stageId,data:{code:error.code}});
    }
    return interrupted;
  }
  ...
  private async enterCheckpoint(run:WorkflowRun,stage:WorkflowStageDefinition){
    ...
    const waiting=await this.withRunLock(run.id,async()=>{
      ...
      if(current.status!=="running")return current;
      const execution=getExecution(current,stage.id);
      if(execution.status!=="ready")return current;
      assertWorkflowStageStatusTransition(execution.status,"running");
      assertWorkflowStageStatusTransition("running","waiting_review");
      assertWorkflowRunStatusTransition(current.status,"waiting_review");
      const at=nowIso();
      const reviewExecution=WorkflowRunSchema.shape.stageExecutions.element.parse({...execution,status:"waiting_review",attempt:execution.attempt+1,attemptId:randomUUID(),startedAt:at,baseProjectRevision:current.lastKnownProjectRevision,error:undefined});
    ...
    if(waiting.status==="waiting_review")await this.activity(run.id,"review-requested",{stageId:stage.id,data:{checkpointId}});
    return waiting;
  }
  ...
  private async completeWorkflow(workflowId:string){
    const completed=await this.withRunLock(workflowId,async()=>{
      ...
      assertWorkflowRunStatusTransition(current.status,"completed");
    ...
    if(completed.status==="completed")await this.activity(workflowId,"workflow-completed");
    return completed;
  }
}
