...
import type {WorkflowStageExecutionContext,WorkflowStageRegistry,WorkflowStageCompletion} from "@/lib/workflows/registry";
...
const hfTransform=undefined;

const mediaImportExecutor=(deps:ProductionWorkflowDependencies)=>({start:async(context:WorkflowStageExecutionContext):Promise<{kind:"completed"}&WorkflowStageCompletion>=>{
...
const mediaProbeExecutor=(deps:ProductionWorkflowDependencies)=>({start:async(context:WorkflowStageExecutionContext):Promise<{kind:"completed"}&WorkflowStageCompletion>=>{
...
const mediaNormalizeExecutor=(deps:ProductionWorkflowDependencies)=>({start:async(context:WorkflowStageExecutionContext):Promise<{kind:"completed"}&WorkflowStageCompletion>=>{
  const project=await latestProject(deps,context);const asset=primarySourceVideo(project,context);
...
const transcribeExecutor=(deps:ProductionWorkflowDependencies)=>({
  start:async(context:WorkflowStageExecutionContext)=>{
    const project=await latestProject(deps,context);
    ...
    return{kind:"job" as const,jobId:job.id};
  },
  reconcileJob:async(context:WorkflowStageExecutionContext,job:JobRecord):Promise<WorkflowStageCompletion>=>{
...
const scriptAnalysisExecutor=(deps:ProductionWorkflowDependencies)=>({start:async(context:WorkflowStageExecutionContext):Promise<{kind:"completed"}&WorkflowStageCompletion>=>{
...
const sceneDetectionExecutor=(deps:ProductionWorkflowDependencies)=>({start:async(context:WorkflowStageExecutionContext):Promise<{kind:"completed"}&WorkflowStageCompletion>=>{
...
const captionGenerationExecutor=(deps:ProductionWorkflowDependencies)=>({start:async(context:WorkflowStageExecutionContext):Promise<{kind:"completed"}&WorkflowStageCompletion>=>{
...
const motionGenerationExecutor=(deps:ProductionWorkflowDependencies)=>({
  start:async(context:WorkflowStageExecutionContext)=>{
    ...
    const job=await reusableJob(deps,context,{type:"hyperframes-render",projectId:project.project.id,input:{expectedRevision:project.project.revision,operationId:operationId(context,"hyperframes-motion"),effectId:recommendation.effectId,props:recommendation.props??{},startFrame:suggestion.startFrame,durationInFrames:suggestion.endFrame-suggestion.startFrame,...(hfTransform?{transform:hfTransform}:{})}});
    return{kind:"job" as const,jobId:job.id};
  },
  reconcileJob:async(context:WorkflowStageExecutionContext,job:JobRecord):Promise<WorkflowStageCompletion>=>{
...
const brollAssemblyExecutor=(deps:ProductionWorkflowDependencies)=>({start:async(context:WorkflowStageExecutionContext):Promise<{kind:"completed"}&WorkflowStageCompletion>=>{
...
const audioAssemblyExecutor=(deps:ProductionWorkflowDependencies)=>({start:async(context:WorkflowStageExecutionContext):Promise<{kind:"completed"}&WorkflowStageCompletion>=>{
  const project=await latestProject(deps,context);const audio=project.tracks.find(track=>track.id==="audio-main")?.clips??[];return{kind:"completed",outputDigest:digest(audio.map(clip=>({id:clip.id,start:clip.startFrame,duration:clip.durationInFrames,enabled:clip.enabled}))};
}});

const timelineAssemblyExecutor=(deps:ProductionWorkflowDependencies)=>({start:async(context:WorkflowStageExecutionContext):Promise<{kind:"completed"}&WorkflowStageCompletion>=>{
...
const previewExecutor=(deps:ProductionWorkflowDependencies)=>({start:async(context:WorkflowStageExecutionContext):Promise<{kind:"completed"}&WorkflowStageCompletion>=>{
  const project=await latestProject(deps,context);const video=project.tracks.find(track=>track.id==="video-main")?.clips.filter(clip=>clip.type==="video"&&clip.enabled)??[];if(video.length===0)throw new Error("Preview requires at least one enabled video clip on video-main.");return{kind:"completed",outputDigest:digest({canvas:project.canvas,projectRevision:project.project.revision,videoClips:video.map(clip=>clip.id),captionCount:project.tracks.find(track=>track.id==="captions-main")?.clips.length??0,motionCount:project.tracks.find(track=>track.id==="motion-main")?.clips.length??0})};
}});

const finalRenderExecutor=(deps:ProductionWorkflowDependencies)=>({
  start:async(context:WorkflowStageExecutionContext)=>{const project=await latestProject(deps,context);const job=await reusableJob(deps,context,{type:"render-final",projectId:project.project.id,input:{assetBaseUrl:deps.assetBaseUrl}});return{kind:"job" as const,jobId:job.id};},
  reconcileJob:async(context:WorkflowStageExecutionContext,job:JobRecord):Promise<WorkflowStageCompletion>=>({artifacts:await jobArtifacts(deps,context.stage.id,job.id),outputDigest:digest({jobId:job.id,attempt:job.attempt,output:job.output})}),
});

export const registerProductionWorkflowStages=(registry:WorkflowStageRegistry,deps:ProductionWorkflowDependencies)=>{
  const registrations:[string,{start:(context:WorkflowStageExecutionContext)=>Promise<any>;reconcileJob?:(context:WorkflowStageExecutionContext,job:JobRecord)=>Promise<WorkflowStageCompletion>}][]=[
