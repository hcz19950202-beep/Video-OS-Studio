...
const RUN_TRANSITIONS:Record<WorkflowRunStatus,ReadonlySet<WorkflowRunStatus>>={
  ...
  waiting_review:new Set(["running","cancelled"]),
  paused:new Set(["running","failed","cancelled","interrupted"]),
  completed:new Set(),
...
export class WorkflowStateTransitionError extends Error{
  ...
  constructor(readonly entity:"run"|"stage",readonly from:string,readonly to:string){super(`Workflow ${entity} cannot transition from ${from} to ${to}.`);this.name="WorkflowStateTransitionError";}
}

export const canTransitionWorkflowRunStatus=(from:WorkflowRunStatus,to:WorkflowRunStatus)=>RUN_TRANSITIONS[from].has(to);
export const assertWorkflowRunStatusTransition=(from:WorkflowRunStatus,to:WorkflowRunStatus)=>{if(!canTransitionWorkflowRunStatus(from,to))throw new WorkflowStateTransitionError("run",from,to);};
...
export const assertWorkflowStageStatusTransition=(from:WorkflowStageStatus,to:WorkflowStageStatus)=>{if(!canTransitionWorkflowStageStatus(from,to))throw new WorkflowStateTransitionError("stage",from,to);};
