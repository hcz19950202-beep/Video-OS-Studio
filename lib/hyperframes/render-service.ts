...
export class HyperFramesRenderService {
  ...
  async renderAndAdd(input: { projectId: string; effectId: string; props: Record<string, unknown>; startFrame: number; durationInFrames: number }): Promise<Project> {
    ...
    project = applyProjectCommand(project, { type: "add-clip", trackId: "motion-main", clip: { id: `motion-${assetId}-${Date.now()}`, type: "motion", engine: "hyperframes", effectId: parsed.effectId, assetId, props: parsed.props, startFrame: input.startFrame, durationInFrames: input.durationInFrames, enabled: true, layer: 20 } });
