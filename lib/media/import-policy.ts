...
export type MediaImportStrategy="native"|"normalize-video"|"normalize-audio"|"asset-only";
...
const AUDIO_NATIVE_EXTENSIONS=new Set([".mp3",".wav",".m4a"]);
...
export const planMediaImport=(fileName:string,mimeType?:string):MediaImportPlan=>{
  ...
  if(AUDIO_EXTENSIONS.has(extension))return{kind:"audio",strategy:AUDIO_NATIVE_EXTENSIONS.has(extension)?"asset-only":"normalize-audio",extension,workingExtension:AUDIO_NATIVE_EXTENSIONS.has(extension)?extension:".m4a",mimeType:resolvedMime};
