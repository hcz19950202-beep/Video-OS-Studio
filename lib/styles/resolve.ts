...
export type ResolvedMotionStyle={props:Record<string,unknown>;transform:MotionTransform;linkedStyleId?:string};
...
export const resolveMotionStyle=(project:Project,clip:Extract<Clip,{type:"motion"}>):ResolvedMotionStyle=>{
  ...
  if(linkedProps.accentColor===undefined&&clip.props.accentColor===undefined&&project.brand.colors.primary){
    props.accentColor=project.brand.colors.primary;
  }
...
export const resolveCaptionStyle=(project:Project,clip:Extract<Clip,{type:"caption"}>):CaptionVisualStyle=>{
  ...
  const base:CaptionVisualStyle={fontFamily:project.brand.typography.captionFont,fill:project.brand.colors.text,...clip.style};
  return{
    ...base,
...
export const createMotionLinkedStyleProperties=(clip:Extract<Clip,{type:"motion"}>)=>({props:{...clip.props},transform:{scale:(clip.transform?.scale??1),opacity:(clip.transform?.opacity??1)}});
export const createCaptionLinkedStyleProperties=(clip:Extract<Clip,{type:"caption"}>)=>({style:{...clip.style},preset:clip.preset,emphasis:clip.emphasis});
