...
const typeRank: Record<VisualClip["type"], number> = {
  video: 0,
  broll: 1,
  motion: 2,
  caption: 3,
...
const transformCss = (value: MotionTransform) =>
  `translate3d(${value.x}px, ${value.y}px, 0) scale(${value.scale}) rotate(${value.rotation ?? 0}deg)`;

const mediaStyle = (
  transform: MotionTransform | undefined,
  fit: "contain" | "cover" = "contain",
): CSSProperties => {
  const value = { ...DEFAULT_MOTION_TRANSFORM, ...(transform ?? {}) };
  return {
    width: "100%",
    height: "100%",
    objectFit: fit,
    transform: transformCss(value),
    transformOrigin: anchorOrigins[value.anchor],
...
const fadeGain = (
  ...
  const a = fadeIn
    ? interpolate(frame, [0, Math.min(fadeIn, duration)], [0, 1], {
        extrapolateLeft: "clamp",
        extrapolateRight: "clamp",
      })
    : 1;
  const start = Math.max(0, duration - Math.min(fadeOut, duration));
  const b = fadeOut
    ? interpolate(frame, [start, duration], [1, 0], {
        extrapolateLeft: "clamp",
        extrapolateRight: "clamp",
      })
    : 1;
...
export const brollMediaKind = (
  assetKind: Asset["kind"] | undefined,
): "image" | "video" => (assetKind === "image" ? "image" : "video");

export const brollRenderRank = (assetKind: Asset["kind"] | undefined) =>
  assetKind === "image" ? 1 : 0;
...
const BrollLayer = ({
  ...
  const style = mediaStyle(clip.transform, clip.fit ?? "cover");
  const renderedStyle = {
    ...style,
    opacity: Number(style.opacity ?? 1) * gain,
  };
  if (brollMediaKind(assetKind) === "image") {
    return <NativeImage src={src} style={renderedStyle} />;
  }
  return (
    <StandardVideo
      src={src}
...
const AudioLayer = ({ clip, src }: { clip: AudioClip; src: string }) => {
  const frame = useCurrentFrame();
  const gain = fadeGain(
    frame,
    clip.durationInFrames,
  ...
  return (
    <Audio
      ...
      volume={clip.muted ? 0 : clip.volume * gain}
    />
  );
};
...
const MotionLayer = ({
  ...
  const style: CSSProperties = {
    transform: transformCss(resolved.transform),
    transformOrigin: anchorOrigins[resolved.transform.anchor],
  ...
  if (clip.engine === "hyperframes") {
    ...
    return src ? (
      <AbsoluteFill style={style}>
        <TransparentVideo src={src} />
      </AbsoluteFill>
    ) : null;
  }
  const effect = EFFECTS_BY_ID[clip.effectId];
  if (!effect) return null;
  const parsed = effect.schema.safeParse(resolved.props);
  if (!parsed.success) return null;
  const Effect = effect.component;
  return (
    <AbsoluteFill style={style}>
      <Effect props={parsed.data} motionSpeed={project.brand.motion.speed} />
    </AbsoluteFill>
  );
};
...
export const MasterComposition: React.FC<MasterCompositionProps> = ({
  ...
  const videos = clips.filter((clip): clip is VideoClip => clip.type === "video");
  const audio = clips.filter((clip): clip is AudioClip => clip.type === "audio");
  const renderOrder = (clip: VisualClip) =>
    clip.type === "broll"
      ? brollRenderRank(
      ...
      : 0;
  const visuals = clips
    .filter((clip): clip is VisualClip => clip.type !== "audio")
    .filter(
      (clip) => !overlay || clip.type === "motion" || clip.type === "caption",
    )
    .sort(
      (a, b) =>
        a.layer - b.layer ||
  ...
  const renderVisual = (clip: VisualClip) => {
    if (clip.type === "video") {
      ...
      return src ? (
        ...
        >
          <StandardVideo
            ...
            style={mediaStyle(clip.transform, clip.fit ?? "contain")}
          />
        </Sequence>
      ) : null;
    }
    ...
    if (clip.type === "motion") {
      return (
        <Sequence
          ...
          durationInFrames={clip.durationInFrames}
        >
          <MotionLayer project={project} clip={clip} assetUrls={assetUrls} />
        </Sequence>
      );
    }
    return (
      ...
      >
        <AbsoluteFill>
          <CaptionOverlay caption={clip} project={project} />
        </AbsoluteFill>
      </Sequence>
    );
  };
  ...
  return (
    <AbsoluteFill
      style={{
        backgroundColor: overlay ? undefined : project.brand.colors.background,
        color: project.brand.colors.text,
        fontFamily: project.brand.typography.bodyFont,
      }}
    >
      ...
      {!overlay && videos.length === 0 && renderMode === "preview" ? (
        ...
        >
          <div style={{ width: "100%" }}>
            <div
              style={{
                color: project.brand.colors.primary,
                fontSize: 34,
                fontWeight: 800,
                letterSpacing: 3,
              }}
      ...
      {renderMode === "preview" ? (
        ...
        >
          {project.canvas.width}×{project.canvas.height} · {project.canvas.fps} fps · f
          {frame}
        </div>
      ) : null}
    </AbsoluteFill>
