...
export const MasterComposition:React.FC<MasterCompositionProps>=({project,assetUrls={},renderMode="preview"})=>{
  ...
  return <AbsoluteFill style={{backgroundColor:overlay?undefined:"#080b0f",color:"#f5f7fa",fontFamily:"Arial, sans-serif"}}>
    {!overlay&&videos.map((clip)=>{const src=assetUrls[clip.assetId];return src?<Sequence key={clip.id} from={clip.startFrame} durationInFrames={clip.durationInFrames}><OffthreadVideo src={src} trimBefore={clip.sourceStartFrame} volume={clip.volume} style={{width:"100%",height:"100%",objectFit:"contain"}}/></Sequence>:null;})}
