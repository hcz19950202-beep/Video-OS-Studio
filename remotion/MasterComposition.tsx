...
const motionStyle=(transform:MotionTransform|undefined):CSSProperties=>{
  const value={...DEFAULT_MOTION_TRANSFORM,...(transform??{})};
  return{
    transform:`translate3d(${value.x}px, ${value.y}px, 0) scale(${value.scale})`,
...
export const MasterComposition:React.FC<MasterCompositionProps>=({project,assetUrls={},renderMode="preview"})=>{
  ...
  return <AbsoluteFill style={{backgroundColor:overlay?undefined:"#080b0f",color:"#f5f7fa",fontFamily:"Arial, sans-serif"}}>
    {!overlay&&videos.map(clip=>{
      const src=assetUrls[clip.assetId];
      return src?<Sequence key={clip.id} from={clip.startFrame} durationInFrames={clip.durationInFrames}><OffthreadVideo src={src} trimBefore={clip.sourceStartFrame} volume={clip.volume} style={{width:"100%",height:"100%",objectFit:"contain"}}/></Sequence>:null;
    })}

    {motions.sort((a,b)=>a.layer-b.layer).map(clip=>{
      if(clip.engine==="hyperframes"){
        const src=clip.assetId?assetUrls[clip.assetId]:undefined;
        return src?<Sequence key={clip.id} from={clip.startFrame} durationInFrames={clip.durationInFrames}><AbsoluteFill style={motionStyle(clip.transform)}><OffthreadVideo src={src} transparent volume={0} style={{width:"100%",height:"100%",objectFit:"contain"}}/></AbsoluteFill></Sequence>:null;
      }
      const effect=EFFECTS_BY_ID[clip.effectId];
      if(!effect)return null;
      const Effect=effect.component;
      const parsed=effect.schema.safeParse(clip.props);
      return parsed.success?<Sequence key={clip.id} from={clip.startFrame} durationInFrames={clip.durationInFrames}><AbsoluteFill style={motionStyle(clip.transform)}><Effect props={parsed.data}/></AbsoluteFill></Sequence>:null;
    })}
