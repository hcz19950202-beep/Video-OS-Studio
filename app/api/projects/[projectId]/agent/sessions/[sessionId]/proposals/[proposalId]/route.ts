import {z} from "zod";
import {AgentProposalApplicationError,AgentProposalNotFoundError,AgentProposalStaleError,AgentSessionIdSchema} from "@/lib/ai";
...
const errorResponse=(error:unknown)=>{
  ...
  if(error instanceof AgentProposalApplicationError)return Response.json({code:error.code,message:error.message,retryable:false,action:"Review the proposal selection and retry."},{status:400});
  ...
  return Response.json({code:"AGENT_PROPOSAL_ACTION_FAILED",message:"Agent proposal action failed.",retryable:true,action:"Reload the latest Project and Agent session before retrying."},{status:500});
};
